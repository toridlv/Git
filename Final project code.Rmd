...
brfss19$CHD <- relevel(brfss19$CHD, ref = "No")
brfss19$ever_smk <- relevel(brfss19$ever_smk, ref = "No")
brfss19
summary(brfss19)
```
...
```{r}
table(brfss19$ex_formal)
table(brfss19$race)
table(brfss19$gen_health)
table(brfss19$ever_smk)
```

##Table 1
```{r}
#Adding variable labels
label(brfss19$ex_formal) <- "Formal exercise"
label(brfss19$age) <- "Age(yrs)"
label(brfss19$sex) <- "Sex"
label(brfss19$race) <- "Race"
label(brfss19$ex_freq) <- "Exercise frequency(per week)"
label(brfss19$ever_smk) <- "Ever smoker"
label(brfss19$CHD) <- "Coronary Heart Disease(CHD)"

#Creating table 1
table1(~ex_formal + age + sex + race + ex_freq + ever_smk | CHD, brfss19, render.missing = NULL)
```

## Binary/Multivariate Logistic Regression analysis of CHD status and exercise type 
```{r}
#Logistic model for CHD (before adjusting)
model_CHD_before <- glm(CHD ~ ex_formal, family = binomial, data = brfss19)
model_CHD_before
#exponentiating model coefficients for odds
exp(coef(model_CHD_before))

#Logistic model for CHD (after adjusting)
model_CHD <- glm(CHD ~ ex_formal + age + sex + race + ex_freq + ever_smk, family = binomial, data = brfss19)
model_CHD
...
```{r}
model_em <- glm(CHD ~ ex_formal + age + sex + race + ex_freq + ever_smk + sex*ex_formal, family = binomial, data = brfss19)
model_em
...
model_em2 <- glm(CHD ~ ex_formal + age + sex + race + ex_freq + ever_smk*ex_formal, family = binomial, data = brfss19)
model_em2
...
#Correlations
OHE <- dummyVars(" ~ .", data = brfss19,sep = "__")
chd2 <- data.frame(predict(OHE,newdata = brfss19))
chd2
corMat <- cor(chd2)
ggcorrplot(corMat)
#Model VIFs
...
boxTidwellage <- glm(CHD ~ age + age.times.logage, data = brfss19, family = "binomial") #Box Tidwell technique, test the assumption of linearity
...
boxTidwellex_freq <- glm(CHD ~ ex_freq + ex_freq.times.logex_freq, data = brfss19, family = "binomial") #Box Tidwell technique, test the assumption of linearity
